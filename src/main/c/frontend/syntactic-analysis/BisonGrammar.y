...
declaration: COLOR IDENTIFIER ASSIGNMENT COLOR_VALUE SEMICOLON			{ $$ = DeclarationSemanticAction(COLOR_NAME, $2, $4, currentCompilerState()); free($2); free($4);}
    | STITCH IDENTIFIER ASSIGNMENT stitch SEMICOLON						{ char * stitch = getStitchValue($4->stitchType); $$ = DeclarationSemanticAction(STITCH_NAME, $2, stitch, currentCompilerState()); free(stitch); free($2);releaseStitch($4); }
    ;
...
argument: IDENTIFIER													{ $$ = ArgumentSemanticAction($1, ITEM_IDENTIFIER, currentCompilerState()); }
    | COLOR_VALUE														{ $$ = ArgumentSemanticAction($1, ITEM_COLOR_VALUE, currentCompilerState()); }
    | stitch															{ $$ = ArgumentSemanticAction($1, ITEM_STITCH, currentCompilerState()); }
    | pattern_use                                                       { $$ = ArgumentSemanticAction($1, ITEM_PATTERN_USE, currentCompilerState()); }
    | OPEN_BRACKET stitch_list CLOSE_BRACKET                            { $$ = ArgumentSemanticAction($2, ITEM_SEQUENCE, currentCompilerState()); }
    ;

pattern_def: PATTERN IDENTIFIER OPEN_PARENTHESIS parameter_list CLOSE_PARENTHESIS { SavePatternToCurrentScope($2, $4, currentCompilerState()); OpenAndFillNewScope($4, currentCompilerState()); } OPEN_BRACE sequence CLOSE_BRACE { CloseLastScope(); } SEMICOLON                                     { $$ = PatternSemanticAction($2, $4, $8);free($2);  }
...
pattern_use: IDENTIFIER OPEN_PARENTHESIS argument_list CLOSE_PARENTHESIS    { $$ = PatternUseSemanticAction($1, $3, currentCompilerState()); free($1); }
           | IDENTIFIER OPEN_PARENTHESIS CLOSE_PARENTHESIS                  { $$ = PatternUseSemanticAction($1, NULL, currentCompilerState());free($1);  }
           ;

pattern_use_in_function: pattern_use                                    { $$ = $1; }
            |   IDENTIFIER                                              { $$ = PatternUseSemanticAction($1, NULL, currentCompilerState()); free($1); }
            ;
...
row_element: stitch                                                     { $$ = $1; }
           ...
           | repeat                                                     { $$ = $1; }
           | mirror                                                     { $$ = $1; }
           | IDENTIFIER                                                 { $$ = ArgumentSemanticAction($1, ITEM_IDENTIFIER, currentCompilerState()); }
            ;
...
row:    row_elements SEMICOLON											{ $$ = RowSemanticAction($1, NULL, ISNOTTURN, false, currentCompilerState()); }
	|	COLOR_VALUE row_elements SEMICOLON								{ $$ = RowSemanticAction($2, $1, ISNOTTURN, false, currentCompilerState());free($1); }
	|	TURN row_elements SEMICOLON										{ $$ = RowSemanticAction($2, NULL, ISTURN, false, currentCompilerState()); }
	|	COLOR_VALUE TURN row_elements SEMICOLON							{ $$ = RowSemanticAction($3, $1, ISTURN, false, currentCompilerState()); free($1);}
    |	IDENTIFIER TURN row_elements SEMICOLON							{ $$ = RowSemanticAction($3, $1, ISTURN, true, currentCompilerState()); free($1);}
    ;
...
repeat: REPEAT OPEN_PARENTHESIS pattern_use_in_function COMMA INTEGER CLOSE_PARENTHESIS								{ $$ = RepeatSemanticAction($3, $5,currentCompilerState()); }
    | REPEAT OPEN_PARENTHESIS OPEN_BRACKET stitch_list CLOSE_BRACKET COMMA INTEGER CLOSE_PARENTHESIS                { PatternUse *anon = PatternUseSemanticAction(NULL, $4, currentCompilerState());  $$ = RepeatSemanticAction(anon, $7,currentCompilerState());}

    ;

mirror: MIRROR OPEN_PARENTHESIS OPEN_BRACKET stitch_list CLOSE_BRACKET COMMA INTEGER CLOSE_PARENTHESIS                { PatternUse *anon = PatternUseSemanticAction(NULL, $4, currentCompilerState());  $$ = MirrorSemanticAction(anon, $7,currentCompilerState()); }
    ;
